...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      const { data, error } = await admin.rpc("get_adult_anamnesis_by_invite_token", { _token: token });
      if (error) {
    ...
    if (req.method === "POST") {
      ...
      const { data, error } = await admin.rpc("submit_adult_anamnesis", {
        _token: token,
      ...
      if (error) {
        ...
        if (msg.includes("missing_full_name")) return json({ error: "Nome completo é obrigatório" }, 400);
