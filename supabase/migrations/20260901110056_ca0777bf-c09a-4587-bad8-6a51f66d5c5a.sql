...
BEGIN
  ...
  IF NEW.plan_name IS DISTINCT FROM OLD.plan_name THEN
    RAISE EXCEPTION 'Not allowed to modify plan_name';
  END IF;
  IF NEW.subscription_started_at IS DISTINCT FROM OLD.subscription_started_at THEN
    RAISE EXCEPTION 'Not allowed to modify subscription_started_at';
  END IF;
  IF NEW.last_payment_at IS DISTINCT FROM OLD.last_payment_at THEN
    RAISE EXCEPTION 'Not allowed to modify last_payment_at';
  END IF;
  IF NEW.next_renewal_at IS DISTINCT FROM OLD.next_renewal_at THEN
    RAISE EXCEPTION 'Not allowed to modify next_renewal_at';
  END IF;
  IF NEW.subscription_notes IS DISTINCT FROM OLD.subscription_notes THEN
    RAISE EXCEPTION 'Not allowed to modify subscription_notes';
  END IF;
